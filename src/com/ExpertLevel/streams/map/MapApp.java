package com.ExpertLevel.streams.map;

import com.ExpertLevel.streams.filtering.Product;
...
public class MapApp {
    ...
    public static void main(String[] args) {
        ...
        List<String> namesCapitalized = names.stream().map(name -> name.substring(0, 1).toUpperCase() + name.substring(1)).collect(Collectors.toList());
        ...
        List<String> cerealNames = cereals.stream().map(Product::getName).collect(Collectors.toList());
        System.out.println("Cereal names:");
        System.out.println(cerealNames);

        List<Product> increasedPriceCereals = cereals.stream()
                .map(product -> new Product(
                        product.getName(), product.getBrand(), product.getPrice() * 1.2, product.getRating()
                )).collect(Collectors.toList());
        System.out.println("Increased cereal price:");
