...
public class PlanetApp {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("The number of planets in database: " + planets.stream().distinct().count());
        ...
        System.out.println("The biggest planet is " + theBiggest.get().getName()
                + " with " + theBiggest.get().getSize() + " km");
        ...
        planets.stream()
                .filter(planet -> PlanetType.SOLID.equals(planet.getType()))
        ...
        System.out.println("The planet that is at the same distance from it's closest star as the Earth from the Sun:");
        planets.stream()
                .sorted(Comparator.comparing(p -> Math.abs(p.getDistanceFromTheClosestStar() - EARTH_SUN_DISTANCE_IN_LIGHT_YEARS)))
                .limit(1)
                .forEach(System.out::println);
    }
}
